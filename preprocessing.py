...
def transform_to_one_hot(data_frame, feature, dummy_na=True):
...
def inf_to(data_frame, feature, fill_val=-1):
    data_frame[feature] = data_frame[feature].apply(lambda elem: fill_val if np.isinf(elem) else elem)
    return data_frame
...
def na_flag_creator(row_data, column_name):
    row_data[column_name] = 1 if np.isnan(row_data[column_name]) else 0
    return row_data
